...
namespace Flextronics.Applications.Library.Schema
{
    ...
    public class SQLServer2000DbObject : IDbObject
{
    // Fields
    ...
    public SQLServer2000DbObject(string DbConnectStr)
    {
        this.cmcfgfile = Application.StartupPath + @"\cmcfg.ini";
        this.connect = new SqlConnection();
    ...
    public SQLServer2000DbObject(bool SSPI, string Ip, string User, string Pass)
    {
        this.cmcfgfile = Application.StartupPath + @"\cmcfg.ini";
        this.connect = new SqlConnection();
        this.connect = new SqlConnection();
    ...
    public List<ColumnInfo> GetColumnInfoList(string DbName, string TableName)
    {
        ...
        builder.Append("Length=CASE WHEN b.name='nchar' THEN a.length/2 WHEN b.name='nvarchar' THEN a.length/2 ELSE a.length END,");
        ...
        while (reader.Read())
        {
            ...
            item.cisNull = reader.GetString(8) == "√";
            item.DefaultVal = reader.GetString(9);
            item.DeText = reader.GetString(10);
            list.Add(item);
        }
        reader.Close();
        return list;
    ...
    public class SQLServer2000DbScriptBuilder : IDbScriptBuilder
{
    // Fields
    private string _dbconnectStr;
    ...
    private SQLServer2000DbObject dbobj = new SQLServer2000DbObject();
    ...
    public string CreateTabScript(string dbname, string tablename)
    {
        ...
        if (columnInfoDt != null)
        {
            ...
            foreach (DataRow row in rowArray)
            {
                ...
                string str16 = str3.Trim();
                if (str16 != null)
                {
                    if ((!(str16 == "varchar") && !(str16 == "char")) && (!(str16 == "nchar") && !(str16 == "nvarchar")))
                    {
                        if ((str16 == "decimal") || (str16 == "numeric"))
                        {
                            goto Label_0281;
                        }
                    }
    ...
    public void CreateTabScript(string dbname, string tablename, string filename, ProgressBar progressBar)
    {
        ...
        if ((columnInfoList != null) && (columnInfoList.Count > 0))
        {
            ...
            foreach (ColumnInfo info in columnInfoList)
            {
                ...
                string str13 = typeName.Trim();
                if (str13 != null)
                {
                    if ((!(str13 == "varchar") && !(str13 == "char")) && (!(str13 == "nchar") && !(str13 == "nvarchar")))
                    {
                        if ((str13 == "decimal") || (str13 == "numeric"))
                        {
                            goto Label_0217;
                        }
                    }
    ...
    public string GetPROCCode(bool Maxid, bool Ishas, bool Add, bool Update, bool Delete, bool GetModel, bool List)
    {
        ...
        plus.AppendLine("* 时间：" + DateTime.Now.ToString());
        //plus.AppendLine("* Made by Flextronics ASP.NET Factroy ");
        plus.AppendLine("******************************************************************/");
        plus.AppendLine("");
        if (Maxid)
        {
    ...
    public string GetSQLInsert(string dbname, string tablename)
    {
        ...
        plus.AppendLine(") VALUES (" + plus2.Value + ")");
        return plus.Value;
    }
