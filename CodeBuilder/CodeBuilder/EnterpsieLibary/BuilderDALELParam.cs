...
namespace Flextronics.Applications.Library.CodeBuilder.EnterpriseLibrary
{
    ...
    public string CreatAdd()
    {
        ...
        foreach (ColumnInfo info in this.Fieldlist)
        {
            ...
            if (!info.IsIdentity)
            {
                ...
                plus4.AppendSpaceLine(3, "db.AddInParameter(dbCommand, \"" + columnName + "\", DbType." + CSToProcType(typeName) + ", Entity." + columnName + ");");
            }
        }
        plus2.DelLastComma();
        plus3.DelLastComma();
    ...
    public string CreatExists()
    {
        ...
        if (this._keys.Count > 0)
        {
            ...
            plus.AppendSpaceLine(3, "Database db = DatabaseFactory.CreateDatabase();");
            plus.AppendSpaceLine(3, "StringBuilder strSql = new StringBuilder();");
            plus.AppendSpace(3, "strSql.Append(\"SELECT COUNT(1) FROM " + this._tablename);
            plus.AppendLine(" WHERE " + this.GetWhereExpression(this.Keys) + "\");");
            plus.AppendSpaceLine(3, "DbCommand dbCommand = db.GetSqlStringCommand(strSql.ToString());");
            plus.Append(this.GetPreParameter(this.Keys));
    ...
    public string CreatGetModel()
    {
        ...
        plus.AppendSpaceLine(3, this.KeysNullTip);
        plus.AppendSpaceLine(3, "StringBuilder strSql=new StringBuilder();");
        plus.AppendSpaceLine(3, "strSql.Append(\"SELECT " + this.Fieldstrlist + "   FROM " + this._tablename + " \");");
        plus.AppendSpaceLine(3, "strSql.Append(\" WHERE " + this.GetWhereExpression(this.Keys) + "\");");
        plus.AppendSpaceLine(3, "Database db = DatabaseFactory.CreateDatabase();");
        plus.AppendSpaceLine(3, "DbCommand dbCommand = db.GetSqlStringCommand(strSql.ToString());");
        plus.Append(this.GetPreParameter(this.Keys));
    ...
    public string CreatUpdate()
    {
       ...
       // plus.AppendSpaceLine(2, "public void Update(" + this.ModelSpace + " model)");
        ...
        foreach (ColumnInfo info in this.Fieldlist)
        {
            ...
            bool isIdentity = info.IsIdentity;
            bool isPK = info.IsPK;
            plus2.AppendSpaceLine(3, "db.AddInParameter(dbCommand, \"" + columnName + "\", DbType." + CSToProcType(typeName) + ",  Entity." + columnName + ");");
