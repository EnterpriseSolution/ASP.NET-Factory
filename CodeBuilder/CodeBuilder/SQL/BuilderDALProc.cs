...
#region  Fields
    ...
    public string CreatAdd()
    {
        ...
        foreach (ColumnInfo info in this.Fieldlist)
        {
            ...
            plus.AppendSpaceLine(5, "new " + this.DbParaHead + "Parameter(\"" + this.preParameter + columnName + "\", " + this.DbParaDbType + "." + CodeCommon.DbTypeLength(this.dbobj.DbType, typeName, length) + "),");
            if (info.IsIdentity)
            {
                num = num2;
            ...
            {
                plus2.AppendSpaceLine(3, string.Concat(new object[] { "parameters[", num2, "].Value = model.", columnName, ";" }));
                num2++;
            }
        }
        plus.DelLastComma();
        ...
        plus.AppendSpaceLine(3, "" + this.DbHelperName + ".RunProcedure(\"" + this.ProcPrefix + this._tablename + "_ADD\",parameters,out rowsAffected);");
        if (((this.dbobj.DbType == "SQL2000") || (this.dbobj.DbType == "SQL2005")) && this.IsHasIdentity)
        {
            plus.AppendSpaceLine(3, string.Concat(new object[] { "return (", this._keyType, ")parameters[", num, "].Value;" }));
        }
        plus.AppendSpaceLine(2, "}");
        return plus.Value;
    }
    ...
    public string CreatDelete()
    {
        ...
        plus.AppendSpaceLine(3, "" + this.DbHelperName + ".RunProcedure(\"" + this.ProcPrefix + this._tablename + "_Delete\",parameters,out rowsAffected);");
        plus.AppendSpaceLine(2, "}");
        return plus.Value;
    }
    ...
    public string CreatExists()
    {
        ...
        if (this._keys.Count > 0)
        {
            ...
            plus.AppendSpaceLine(3, "int rowsAffected;");
            plus.AppendLine(this.GetPreParameter(this.Keys));
            plus.AppendSpaceLine(3, "int result= " + this.DbHelperName + ".RunProcedure(\"" + this.ProcPrefix + this._tablename + "_Exists\",parameters,out rowsAffected);");
    ...
    public string CreatGetModel()
    {
        ...
        plus.AppendSpaceLine(3, "DataSet ds= " + this.DbHelperName + ".RunProcedure(\"" + this.ProcPrefix + this._tablename + "_GetModel\",parameters,\"ds\");");
    ...
    public string CreatUpdate()
    {
        ...
        foreach (ColumnInfo info in this.Fieldlist)
        {
            ...
            string length = info.Length;
            plus.AppendSpaceLine(5, "new " + this.DbParaHead + "Parameter(\"" + this.preParameter + columnName + "\", " + this.DbParaDbType + "." + CodeCommon.DbTypeLength(this.dbobj.DbType, typeName, length) + "),");
            plus2.AppendSpaceLine(3, string.Concat(new object[] { "parameters[", num, "].Value = model.", columnName, ";" }));
            num++;
        }
        plus.DelLastComma();
        plus.AppendLine("};");
        plus.AppendLine(plus2.Value);
        plus.AppendSpaceLine(3, "" + this.DbHelperName + ".RunProcedure(\"" + this.ProcPrefix + this._tablename + "_Update\",parameters,out rowsAffected);");
        plus.AppendSpaceLine(2, "}");
        return plus.Value;
    }
