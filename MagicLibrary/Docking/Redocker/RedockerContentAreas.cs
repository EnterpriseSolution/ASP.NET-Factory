...
namespace Junxian.Magic.Docking
{
    ...
    public class RedockerContentAreas : RedockerContent
    {
        ...
        private void GenerateHotAreas()
        {
			...
			_hotAreas.Add(new HotAreaOutside(Squares, _dockingManager, _outsideRect, VectorV, VectorH));

			// With InsideFill enable we cannot do inside fill operations
			if (!_dockingManager.InsideFill)
				_hotAreas.Add(new HotAreaInside(Squares, _dockingManager,  _insideRect, VectorV, VectorH));
