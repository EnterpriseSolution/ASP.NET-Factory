...
namespace Junxian.Magic.Docking
{
    ...
    public abstract class WindowDetailCaption : WindowDetail, IMessageFilter
    {
		...
		/// <summary>
		/// Base class maximize button.
		/// </summary>
        protected ButtonWithStyle _maxButton;

		/// <summary>
		/// Base class close button.
		/// </summary>
        protected ButtonWithStyle _closeButton;

		/// <summary>
		/// Base class hide button.
		/// </summary>
        protected ButtonWithStyle _hideButton;

		/// <summary>
		/// Base class redocker reference.
		/// </summary>
        protected RedockerContent _redocker;

		/// <summary>
		/// Base class access to min/max interface.
		/// </summary>
        protected IZoneMaximizeWindow _maxInterface;

		/// <summary>
		/// Base class value indicating if close button is to be shown.
		/// </summary>
        protected bool _showCloseButton;

		/// <summary>
		/// Base class value indicating if hide button is to be shown.
		/// </summary>
        protected bool _showHideButton;

		/// <summary>
		/// Base class value indicating if hide button should be ignored.
		/// </summary>
        protected bool _ignoreHideButton;

		/// <summary>
		/// Base class value indicating if auto hide button should be ignored.
		/// </summary>
        protected bool _pinnedImage;

        /// <summary>
        /// Base class attribute for drawing active images.
        /// </summary>
        protected static ImageAttributes _activeAttr = new ImageAttributes();

		/// <summary>
		/// Base class attribute for drawing inactive images.
		/// </summary>
    ...
    public class WindowDetailCaptionPlain : WindowDetailCaption
    {
		...
		static WindowDetailCaptionPlain()
		{
            ...
            _imagesPlain = ResourceHelper.LoadBitmapStrip(Type.GetType("Junxian.Magic.Docking.WindowDetailCaptionPlain"),
                                                         "Junxian.Magic.Resources.ImagesCaptionPlain.bmp",
    ...
    public class WindowDetailCaptionIDE : WindowDetailCaption
    {
		...
		{
            // Create a strip of images by loading an embedded bitmap resource
            _imagesIDE = ResourceHelper.LoadBitmapStrip(Type.GetType("Junxian.Magic.Docking.WindowDetailCaptionIDE"),
                                                        "Junxian.Magic.Resources.ImagesCaptionIDE.bmp",
                                                        new Size(_imageWidth, _imageHeight),
                                                        new Point(0,0));
            ...
            if (DockLeft)
			{
								...
								inactiveBrush = new SolidBrush(DockingManager.InactiveTextColor))
				{
					...
					if (focused)
					{
						...
						{
							rectCaption.Y += _closeButton.Height;
							rectCaption.Height -= _closeButton.Height;
						}
					...
					else
					{
						...
						using (Pen dark = new Pen(ControlPaint.LightLight(DockingManager.InactiveTextColor)))
						{
							...
							// Reduce the width to account for close button
							rectCaption.Y += _closeButton.Height + ButtonSpacer;
							rectCaption.Height -= _closeButton.Height + ButtonSpacer;

							// Reduce width to account for the optional maximize button
							if ((_maxButton != null) && (_maxButton.Visible))
							{
								rectCaption.Y += _maxButton.Height;
								rectCaption.Height -= _maxButton.Height;
							}
			...
			else
			{
								...
								inactiveBrush = new SolidBrush(DockingManager.InactiveTextColor))
				{
					...
					if (focused)
					{
						...
						e.Graphics.DrawString(this.Text, DockingManager.CaptionFont, activeTextBrush, rectCaption);
					}
					else
					{
						// Fill the entire background area
    ...
    public class WindowDetailCaptionIDE2005 : WindowDetailCaptionIDE
    {
		...
		{
			// Create the InertButton
			_maxButton = new IDE2005Button(this);
        ...
        {
			// Define the ImageList and which ImageIndex to show initially
			_closeButton = new IDE2005Button(this);
			_closeButton.BackColor = Color.Transparent;
			_hideButton = new IDE2005Button(this);
    ...
    public class WindowDetailCaptionOffice2003 : WindowDetailCaption
    {
		...
		{
            // Create a strip of images by loading an embedded bitmap resource
            _imagesOffice = ResourceHelper.LoadBitmapStrip(Type.GetType("Junxian.Magic.Docking.WindowDetailCaptionIDE"),
														   "Junxian.Magic.Resources.ImagesCaptionOffice2003.bmp",
														   new Size(_imageWidth, _imageHeight),
														   new Point(0,0));
		}
		...
		{
			// Create the InertButton
			_maxButton = new Office2003Button(this);
		}
		...
		protected override void CreateButtons()
        {
			...
			{
				// Define the ImageList and which ImageIndex to show initially
				_closeButton = new Office2003Button(this);
				_closeButton.Image = Images.Images[(int)ImageIndex.Close];
				_closeButton.Style = VisualStyle.Office2003;

				_hideButton = new Office2003Button(this);
				_hideButton.Image = Images.Images[(int)ImageIndex.AutoHide];
		...
		protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (_dockLeft)
			{
				...
				// Reduce the height to account for close button
				rectCaption.Y += _closeButton.Width;
				rectCaption.Height -= _closeButton.Width;
			...
			else
			{
				...
				// Reduce the width to account for close button
				rectCaption.Width -= _closeButton.Width + _xInset + _spotWidth;
    ...
    /// </summary>
    internal class Office2003Button : ButtonWithStyle
    {
		// Instance fields
		...
		public Office2003Button(WindowDetailCaptionOffice2003 caption)
		{
    ...
    /// Derived class to draw the background to match the caption.
    /// </summary>
    internal class IDE2005Button : ButtonWithStyle
    {
		// Instance fields
		...
		/// </summary>
		/// <param name="caption"></param>
		public IDE2005Button(WindowDetailCaptionIDE2005 caption)
		{
			// Remember back pointer
			_caption = caption;
		}
