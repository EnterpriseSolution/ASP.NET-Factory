...
namespace Junxian.Magic.Docking
{
    ...
    public class TabStub : UserControl
    {
		private class DrawTab
		{
            ...
            private Junxian.Magic.Controls.TabPage _tabPage;

			public DrawTab(Junxian.Magic.Controls.TabPage tabPage, Rectangle drawRect, int index)
			{
				_index = index;
				_tabPage = tabPage;
				_drawRect = drawRect;
			...
			public Junxian.Magic.Controls.TabPage TabPage	{ get { return _tabPage; } }
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			...
			foreach(DrawTab dt in _drawTabs)
			{
				...
				Junxian.Magic.Controls.TabPage page = dt.TabPage;
		...
		private void ResizeControl()
		{
			...
			if (!IsDisposed)
			{
				...
				{
					foreach(Junxian.Magic.Controls.TabPage page in _tabPages)
					{
			...
			using(Graphics g = this.CreateGraphics())
			{
				...
				for(int index=0; index<_tabPages.Count; index++)
				{
					...
					if ((index ==_selectedIndex) || allText)
					{
						...
						{
							// Find width of the requested text
							SizeF dimension = g.MeasureString(_tabPages[index].Title, this.Font);
		...
		private void OnInsertedPage(int index, object value)
		{
			...
			Junxian.Magic.Controls.TabPage page = value as Junxian.Magic.Controls.TabPage;
