...
namespace Junxian.Magic.Controls
{
	...
	public class GroupNodeVC : DefaultNodeVC
	{
		...
		static GroupNodeVC()
		{
			...
			_upArrow = ResourceHelper.LoadBitmap(typeof(GroupNodeVC), "Junxian.Magic.Controls.TitleBar.ImageUp.bmp", Point.Empty);
			_downArrow = ResourceHelper.LoadBitmap(typeof(GroupNodeVC), "Junxian.Magic.Controls.TitleBar.ImageDown.bmp", Point.Empty);
			...
			if (IsRootNode(tc, n))
			{
				...
				if ((rect.Width > 0) && (rect.Height > 0))
				{
					...
					if (tc.GroupColoring != GroupColoring.ControlProperties)
					{
						...
						GetGroupColouringOffice2003(tc, selectedNode, hotNode, ref top, ref bottom); 
						
						// Fill background in required gradient colour
						using(Brush backBrush = new LinearGradientBrush(rect, top, bottom, 90f))
		...
		public virtual void GetGroupColouringOffice2003(TreeControl tc,
														bool selectedNode, 
														bool hotNode, 
														ref Color top, 
														ref Color bottom)
		...
		public virtual Rectangle DrawGroupBorder(TreeControl tc, Node n, Graphics g, Rectangle rect)
		{
			...
			if (tc.GroupBorderStyle != GroupBorderStyle.None)
			{
				...
				{				
					...
					{
						// Draw a separator line at top of the area
						using(Pen bottomPen = new Pen(tc.ColorDetails.ActiveBorderColor))
