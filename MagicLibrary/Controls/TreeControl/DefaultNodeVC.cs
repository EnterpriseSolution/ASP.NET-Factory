...
namespace Junxian.Magic.Controls
{
	...
	{
		/// <summary>
		/// Cache a StringFormat instance for reduce resource usage
		/// </summary>
			...
			if (ClickPointInNode(tc, n, rect, pt))
			{
				...
				if (button == MouseButtons.Right)
				{
					...
					if (tc.OnShowContextMenuNode(n))
					{
						...
						ContextMenu cm = tc.ContextMenuNode;
		...
		{
			// Does the tree allow selectable nodes and the node itself is selectable
