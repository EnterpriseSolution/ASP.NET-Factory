	...
	{	
		...
		static TreeControl()
		{
			...
			_indicatorImages = ResourceHelper.LoadBitmapStrip(typeof(TreeControl), 
															  "Junxian.Magic.Resources.Indicators.bmp", 
		...
		{
			// Generate the deselect event for each entry
			foreach(Node n in _selected.Keys)
				OnAfterDeselect(n);
		...
		[Editor("Junxian.Magic.Controls.NodeCollectionEditor", typeof(UITypeEditor))]
		...
		/// <summary>
		/// Gets the context menu to use on right clicking a Node.
		/// </summary>
		[Category("Nodes")]
		[Description("ContextMenu to show on right clicking a Node.")]
		[DefaultValue(null)]
		...
		/// <summary>
		/// Gets the context menu to use on right clicking outside of any Node.
		/// </summary>
		[Category("Nodes")]
		[Description("ContextMenu to show on right clicking outside of any Node.")]
		[DefaultValue(null)]
		...
		public void SetTreeControlStyle(TreeControlStyles style)
		{
			switch(style)
			{
				...
				case TreeControlStyles.Group:
					...
					ViewControllers = ViewControllers.Group;
					VerticalGranularity = VerticalGranularity.Pixel;
					GroupFont = new Font(this.Font.Name, this.Font.Size + 1, FontStyle.Bold);
		...
		protected override void OnMouseWheel(MouseEventArgs e)
		{
			...
			if (EnableMouseWheel && _vBar.Enabled && _vBar.Visible)
			{
				...
				int newValue = _vBar.Value - detents;
		...
		protected override void OnMouseUp(MouseEventArgs e)
		{
			...
			{
				if (!upNode.VC.MouseUp(this, upNode, e.Button, HotPoint))
				{
					// Only right mouse button will show context menu
					if (e.Button== MouseButtons.Right)
					{
		...
		protected override void OnDragEnter(DragEventArgs drgevent)
		{
			...
			{
				// Generate an enter event for the remainder area
		...
		internal void DeselectNode(Node n, bool recurse)
		{
			...
			if (_selected.ContainsKey(n))
			{
				...
				// Notify change in selection has completed
				OnAfterDeselect(n);
			...
			if (_lastSelectedNode != null)
			{
				...
				if (IsNodeDisplayed(_lastSelectedNode))
				{
					...
					{	
						...
						// Generate event for each node removed
					...
					if (test != null)
					{
						...
						if (Tooltips && (test.Cache != null) && (test.VC != null))
						{
								...
								(bounds.Right > _drawRectangle.Right))
							{
								...
								_tooltip = new PopupTooltipSingle(VisualStyle.Plain, test.GetNodeFont());
