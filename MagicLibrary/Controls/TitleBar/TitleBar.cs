...
namespace Junxian.Magic.Controls
{
	...
	public class TitleBar : Control
	{
		...
		static TitleBar()
		{
			...
			Image up = ResourceHelper.LoadBitmap(typeof(TitleBar), "Junxian.Magic.Controls.TitleBar.ImageUp.bmp", Point.Empty);
			Image down = ResourceHelper.LoadBitmap(typeof(TitleBar), "Junxian.Magic.Controls.TitleBar.ImageDown.bmp", Point.Empty);
			Image left = ResourceHelper.LoadBitmap(typeof(TitleBar), "Junxian.Magic.Controls.TitleBar.ImageLeft.bmp", Point.Empty);
			Image right = ResourceHelper.LoadBitmap(typeof(TitleBar), "Junxian.Magic.Controls.TitleBar.ImageRight.bmp", Point.Empty);
			Image pinned = ResourceHelper.LoadBitmap(typeof(TitleBar), "Junxian.Magic.Controls.TitleBar.Pinned.bmp", Point.Empty);
			Image unpinned = ResourceHelper.LoadBitmap(typeof(TitleBar), "Junxian.Magic.Controls.TitleBar.Unpinned.bmp", Point.Empty);
		...
		private bool ShouldSerializeGradientDirection()
		{
			if (Style == VisualStyle.IDE)
				return (GradientDirection != GradientDirection.BottomToTop);
			else if ((Style == VisualStyle.Office2003) ||
		...
		{
			if (Style == VisualStyle.IDE)
				return (GradientColoring != GradientColoring.BackToGradientColor);
			else
		...
		{
			if (Style == VisualStyle.IDE)
				GradientColoring = GradientColoring.BackToGradientColor;
			else
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			...
			using(StringFormat sf = new StringFormat())
			{
				...
				// Draw the string aligned and trimmed
		...
		private Color CalcTextColor()
		{
			...
			// Office2003 style can override starting colors
			if ((Style == VisualStyle.Office2003) || (Style == VisualStyle.Office2003))
			{
				// If active, use dark colors
				if (Active)
				{
					// Use Office2003 colors unless default color changed
		...
		private void DrawBackground(PaintEventArgs e)
		{
			...
			Color end = backColor;
			bool ignoreColoring = (Style == VisualStyle.Plain);
