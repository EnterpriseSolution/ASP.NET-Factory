...
namespace Junxian.Magic.Controls
{
    ...
    [Designer(typeof(Junxian.Magic.Controls.TabControlDesigner))]
    public class TabControl : Panel
    {
		...
		private static int[,] _position = { {3, 1, 1, 1, 1, 2, 1, 1, 2, 1, 3, 2, 5},		// IDE
											{6, 2, 2, 3, 3, 1, 1, 0, 1, 1, 2, 0, 5},		// Plain
											{1, 1, 0, 2, 3, 2, 1, 1, 2, 1, 3, 2, 0},		// Office2003
											{1, 1, 0, 1, 3, 2, 1, 1, 2, 1, 5, 2, 0} };		// IDE2005
        ...
        private static int MULTIBOX_ADJUST = 2;
		private static int AUTOSCROLL_VECTOR = 20;
		private static int OFFICE2003_HEADER_INSET = 5;
		...
		private bool _ide2005GradBack;			// Should ide2005 specific elements be graduated
		private bool _ide2005TabJoined;			// Should the active tab look like it joins the main area
		private bool _officeDockSides;			// Place special fading borders around docking in office 2003
        ...
        private PopupMenu _contextMenu;			// Context menu to show on right mouse up
        ...
        private OfficeStyle _officeStyle;		// which Office2003 style variation to use
		...
		/// <summary>
		/// Occurs just before a context menu is shown.
		/// </summary>
		[Category("Behavior")]
        ...
        static TabControl()
        {
            ...
            _internalImages = ResourceHelper.LoadBitmapStrip(Type.GetType("Junxian.Magic.Controls.TabControl"),
                                                             "Junxian.Magic.Resources.ImagesTabControl.bmp",
        ...
        public TabControl()
        {
			...
			_ide2005Style = IDE2005Style.Enhanced;
                ...
                if (_style != value)
                {
					...
					switch(_style)
					{
						...
						case VisualStyle.Plain:
							...
							_leftArrow.Style = VisualStyle.Plain;
							_rightArrow.Style = VisualStyle.Plain;
							_activeArrow.Style = VisualStyle.Plain;
							break;
						case VisualStyle.Office2003:
							_closeButton.Style = VisualStyle.Office2003;
							_leftArrow.Style = VisualStyle.Office2003;
							_rightArrow.Style = VisualStyle.Office2003;
							_activeArrow.Style = VisualStyle.Office2003;
							break;
						case VisualStyle.IDE2005:
							_closeButton.Style = VisualStyle.IDE2005;
							_leftArrow.Style = VisualStyle.IDE2005;
							_rightArrow.Style = VisualStyle.IDE2005;
							_activeArrow.Style = VisualStyle.IDE2005;
							break;
					}
		...
		/// <summary>
		/// Gets or sets the variation to use when drawing in Office2003 style.
		/// </summary>
		[Category("Office2003")]
		...
		/// <summary>
		/// Gets and sets the context menu to be used on right mouse click.
		/// </summary>
        [Category("Behavior")]
		...
		/// <summary>
		/// Decide if the context popup menu needs to be persisted.
		/// </summary>
		/// <returns>Should context popup menu be persisted.</returns>
		...
		/// <summary>
		/// Resets the ContextPopupMenu property to its default value.
		/// </summary>
		...
		/// <summary>
		/// Gets and sets the value indicating Office2003 style should added extra dock specific fade lines.
		/// </summary>
		[Browsable(false)]
		...
		private int GetExtraInset()
		{
			if (_style == VisualStyle.Office2003)
			{
				if (_officeHeaderBorder)
					return OFFICE2003_HEADER_INSET;
				else
		...
		/// <summary>
		/// Gets and sets a value indicating that in Office2003 style a single pixel border is drawn around the entire control.
		/// </summary>
		[Category("Office2003")]
		[DefaultValue(true)]
		...
		/// <summary>
		/// Gets and sets a value indicating that in Office2003 style a single pixel border is drawn around the header control.
		/// </summary>
		[Category("Office2003")]
		[DefaultValue(false)]
		...
		/// <summary>
		/// Gets and sets a value indicating that in IDE2005 styles should drawn using gradient background where appropriate.
		/// </summary>
		[Category("IDE2005")]
		[DefaultValue(true)]
		public virtual bool IDE2005GradBack
		{
			...
			set 
			{
				if (_ide2005GradBack != value)
				{
					...
					_closeButton.IDE2005GradBack = _ide2005GradBack;
					_leftArrow.IDE2005GradBack = _ide2005GradBack;
					_rightArrow.IDE2005GradBack = _ide2005GradBack;
					_activeArrow.IDE2005GradBack = _ide2005GradBack;
					Recalculate();
					Invalidate();
				}
			}
        ...
        protected virtual void OnActiveArrow(object sender, EventArgs e)
        {
			...
			if (_tabPages.Count > 0)
			{
				// Use a PopupMenu for displaying the options
				using(PopupMenu popup = new PopupMenu())
				{
					// Use same display styles as ourself
					popup.Style = this.Style;

					// Create a menu command per tab page
					foreach(TabPage page in _tabPages)
					{
						MenuCommand mc = new MenuCommand(page.Title);
						...
						{
							// Then use same image in the popup
							mc.ImageIndex = page.ImageIndex;

							if (page.ImageList != null)
								mc.ImageList = page.ImageList;
							else
								mc.ImageList = ImageList;
						}
						...
						mc.Click += new EventHandler(OnActiveTabClick);
					...
					popup.TrackPopup(PointToScreen(new Point(_activeArrow.Left, _activeArrow.Bottom + 5)));

					// Cleanup the menu commands
					foreach(MenuCommand mc in popup.MenuCommands)
					{
						mc.Click -= new EventHandler(OnActiveTabClick);
						mc.Tag = null;
					}
		...
		private void OnActiveTabClick(object sender, EventArgs e)
		{
			...
			TabPage page = mc.Tag as TabPage;
        ...
        protected virtual void Recalculate()
        {
			...
			if (!_leftMouseDownDrag)
			{
				...
				else if ((_officePixelBorder && (_style == VisualStyle.Office2003)) ||
				...
				else if (_officeDockSides && (_style == VisualStyle.Office2003) && !_positionAtTop)
				{
					bool vertical = false;
		...
		protected virtual void DrawOuterBorders(Graphics g)
		{
			...
			// Office2003 needs to use theme specific colors
			if (((_style == VisualStyle.Office2003) ||
				 (_style == VisualStyle.IDE2005)) && _defaultColor)
			{
				tabsAreaColor = Office2003BackColor;
				pageAreaColor = Office2003PageColor;
			}
			...
			{
				Color ideBackLeft = base.BackColor;
				Color ideBackRight = base.BackColor;
			
				if (_defaultColor)
				{
					// Define the appropriate colours
					ideBackLeft = IDE2005BackLeftColor;
					ideBackRight = IDE2005BackRightColor;
				}

				CommandDraw.DrawGradientBackground(g, this, ideBackLeft, ideBackLeft, 
												   ideBackRight, _ide2005GradBack, _tabsAreaRect);
			}
			else
			{				
		...
		/// Gets the active color for Office2003 style.
		/// </summary>
		protected virtual Color Office2003PageColor
		{
			get 
			{ 
				switch(_officeStyle)
		...
		/// <summary>
		/// Gets the back color for Office2003 style.
		/// </summary>
		protected virtual Color Office2003BackColor
		{
			get 
			{ 
				if (_defaultColor)
            ...
            if (_appearance != VisualAppearance.MultiBox)
            {
                ...
                switch(_style)
                {
                    ...
                    case VisualStyle.IDE:
                                   ...
                                   backColor = new Pen(base.BackColor))
                        {
                            ...
                            if (_positionAtTop)
                            {
                                ...
                                if (_idePixelBorder)
                                {
                                    g.DrawRectangle(dark, 0, 0, this.Width - 1, this.Height - 1);
                                    indent++;
                                }
                                else
                                {
                                    ...
                                    {
                                        // Draw top border
                                        g.DrawLine(dark, 0, _tabsAreaRect.Top, _tabsAreaRect.Width, _tabsAreaRect.Top);
                                    }
                                }
                                
                                // Draw bottom border
                                if (!hiddenPages)
                                    g.DrawLine(lightlight, indent, 
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        // Draw bottom border
                                        g.DrawLine(backColor, 0, _tabsAreaRect.Bottom - 1, _tabsAreaRect.Width, _tabsAreaRect.Bottom - 1);
                                    }
                                }
					...
					case VisualStyle.IDE2005:
                        ...
                        border = Color.FromArgb(105, 161, 191);
                ...
                if (xDraw < xMax)
                {
                    ...
                    else
				    {
						...
						else
						{
							...
							{
								// Otherwise we use default, but that depends if dimming is needed
								if (_dimUnselected && ((_style != VisualStyle.Office2003) || (_style != VisualStyle.IDE2005)))
            ...
            else
            {
                ...
                switch(_style)
                {
					...
					case VisualStyle.IDE2005:
						...
						break;				
					case VisualStyle.Office2003:
						DrawOffice2003TabBorder(page, g, rectTab);
						break;
				}
            }
        }
        ...
        protected virtual void DrawMultiBoxBorder(TabPage page, Graphics g, Rectangle rectPage)
        {
            if (page.Selected)
            {
				...
				// Use different colours for office 2003
				if (((_style == VisualStyle.Office2003) ||
					 (_style == VisualStyle.IDE2005)) && _defaultColor)
				{
					ll = _colorDetails.MenuBackColor;
...
;
				...
				using(Pen borderPen = new Pen(b))
				{
                    ...
                    if (page.Selected)
                    {
                        ...
                        iat = Color.FromArgb(252, 253, 254); ;
                        iab = Color.FromArgb(210, 230, 250); ;
                    }
                    else
                    {
                        //fillBrush = new LinearGradientBrush(fillRect, iat, iab, 90f);
                        iat = Color.FromArgb(236, 246, 252); ;
                        iab = Color.FromArgb(151, 179, 209); ;
                    }
					...
					if (_positionAtTop)
					{
                        ...
                        Point[] points = new Point[] {  new Point(rectPage.Left, rectPage.Bottom),
                                                        new Point(rectPage.Left, rectPage.Top + 10),
                                                        new Point(rectPage.Left + 20, rectPage.Top),
                                                        new Point(rectPage.Right - 2, rectPage.Top),
                                                        new Point(rectPage.Right, rectPage.Top + 2),
                                                        new Point(rectPage.Right, rectPage.Bottom)
                                                    };
                        ...
                        //{
                            ...
                            g.DrawLine(borderPen, rectPage.Left, rectPage.Bottom, rectPage.Left, rectPage.Top + 10);
						    g.DrawLine(borderPen, rectPage.Left, rectPage.Top + 10, rectPage.Left + 20, rectPage.Top);
                        //}
                        //else
                        //{
                        //    g.DrawLine(borderPen, rectPage.Left - 15, rectPage.Bottom, rectPage.Left - 15, rectPage.Top + 10);
                        //    g.DrawLine(borderPen, rectPage.Left - 15, rectPage.Top + 10, rectPage.Left + 20, rectPage.Top);
                        //}

                        //g.DrawLine(borderPen, rectPage.Left, rectPage.Top + 2, rectPage.Left + 2, rectPage.Top);
                        //g.DrawArc(borderPen, rectPage.Left + 15, rectPage.Top + 10, 5, 5, 45, 45);
						...
						g.DrawLine(borderPen, rectPage.Left + 20, rectPage.Top, rectPage.Right - 2, rectPage.Top);
					}
					else
					{
						if (page.Selected && _ide2005TabJoined)
		...
		/// <summary>
		/// Draw the border around a tab header in Office2003 style.
		/// </summary>
		/// <param name="page">TabPage to draw border for.</param>
		/// <param name="g">Graphics object to use in drawing operations.</param>
		/// <param name="rectPage">Inner rectangle left after drawing border.</param>
		protected virtual void DrawOffice2003TabBorder(TabPage page, Graphics g, Rectangle rectPage)
        {
			// Define default colors
			Color dd = _backDarkDark;
			Color a = Office2003ActiveColor;
			Color ia = ControlPaint.Light(base.BackColor);
			...
			{
				// Define the apropriate colours for office themes
				ia = Office2003BackColor;
				dd = _colorDetails.ActiveBorderColor;
				sd = _colorDetails.SepDarkColor;
				sl = _colorDetails.SepLightColor;
				ed = _colorDetails.TrackLightLightColor1;
				el = _colorDetails.TrackLightLightColor2;
			}
		...
		public virtual void SelectPage(TabPage page)
        {
            page.Selected = true;
			...
			if (_tabsAreaRect.Contains(mousePos))
			{
				...
				if ((_mouseDownIndex >= 0) && (_mouseDownIndex < _tabPages.Count))
				{
					...
					if (Control.MouseButtons == MouseButtons.Left)
					{
						...
						// If we have a pixel border and in IDE mode
						if (IDEPixelBorder && (Style == VisualStyle.IDE))
						{
							// Then we ignore dragging attempt from pressing on the border
                ...
                else
                {
					...
					if (!_multiline && _allowDragReorder)
					{
						...
						if (!DragOutside || _draggingInside)
						{
							...
							TabPage pageOver = TabPageFromDraggingPoint(new Point(e.X, e.Y));
		...
		/// <summary>
		/// Raises the PopupMenuDisplay event.
		/// </summary>
		...
		private void ShowTooltip(string text)
		{
			...
			_toolTip = new PopupTooltipSingle(this.Style);
				
			// Define string for display
			_toolTip.ToolText = text;
			...
			try
			{
				...
				else
				{
					...
					if (_rightArrow.Enabled != _rightScroll)
					{
						...
						if (_leftScroll)
							_leftArrow.Image = _internalImages.Images[(int)ImageStrip.RightEnabled];
						else
							_leftArrow.Image = _internalImages.Images[(int)ImageStrip.RightDisabled];
					}
    ...
    internal class TabButtonWithStyle : ButtonWithStyle
    {
		...
		{
			if ((_tabControl != null) && 
				(_tabControl.Style == VisualStyle.IDE2005) &&
				(BackColor == Color.Transparent))
			{
				// Use helper function to draw background correctly
				CommandDraw.DrawGradientBackground(pevent.Graphics, this, _tabControl.ColorDetails.BaseColor1,
												   _tabControl.ColorDetails.BaseColor1, _tabControl.ColorDetails.BaseColor2,
												   _tabControl.IDE2005GradBack);
			}
			else
	...
	/// <summary>
	/// Specifies how to draw the Office2003 style
	/// </summary>
