...
namespace Junxian.Magic.Common
{
	...
	public class CommandDraw
	{
		...
		static CommandDraw()
		{
			...
			Bitmap checkBitmap = ResourceHelper.LoadBitmap(typeof(CommandDraw), "Junxian.Magic.Resources.MenuCheckMark.bmp", new Point(0,0));
			Bitmap radioCheckBitmap = ResourceHelper.LoadBitmap(typeof(CommandDraw), "Junxian.Magic.Resources.MenuRadioCheckMark.bmp", new Point(0,0));
			Bitmap subMenuBitmap = ResourceHelper.LoadBitmap(typeof(CommandDraw), "Junxian.Magic.Resources.MenuSubMenuMark.bmp", new Point(0,0));
											 ...
											 bool pushed, bool staticButton, bool drawBorder)
		{
			...
			DrawButtonCommandInside(g, style, direction, drawRect, state, 
									...
									text, image, imageAttr, staticButton);
												 ...
												 Color trackBorder, ButtonStyle buttonStyle, bool pushed)
		{
			...
			switch(style)
			{
				...
				case VisualStyle.IDE:
					...
					// but not being hottracked, pressed or open then we draw in a light color
					if ((buttonStyle == ButtonStyle.ToggleButton) && pushed && (state == ItemState.Normal))
					{
						// Draw background with back color
						using(SolidBrush backBrush = new SolidBrush(trackLightLight1))
							g.FillRectangle(backBrush, drawRect);
					}
					else
					{				
						...
						switch(state)
						{
							...
							case ItemState.Open:
								using(SolidBrush trackBrush = new SolidBrush(trackLight1))
									g.FillRectangle(trackBrush, drawRect);
								break;
							case ItemState.Pressed:
								using(SolidBrush trackBrush = new SolidBrush(trackBase1))
									g.FillRectangle(trackBrush, drawRect);
								break;
							default:
								// Should never happen!
				...
				else
				{
					...
					else
					{
						...
						if (total <= required)
						{
							...
							DrawCommandText(g, style, direction, state, enabled, textRect, 
											font, textColor, baseColor, text);
						}
						else
						{
		...
		public static void DrawCommandText(Graphics g, VisualStyle style, LayoutDirection direction,
										   ...
										   string text)
		{
			// Create default format settings to show text in centre
			using(StringFormat format = new StringFormat())
			{
				...
				if (enabled)
				{
					...
					// In IDE style when the button is pressed the text lightens
					if ((style == VisualStyle.IDE) && (state == ItemState.Pressed))
						drawColor = ControlPaint.Light(drawColor);

					// If Plain style when button is pressed then draw down and right
					if ((style == VisualStyle.Plain) && (state == ItemState.Pressed))
						drawRect.Offset(1, 1);
					
					// Create a floating version of the rectangle
					RectangleF drawRectF = new RectangleF(drawRect.Left, drawRect.Top, 
			...
			switch(style)
			{
				...
				case VisualStyle.IDE:
					...
					else
					{
						...
						{
							case ItemState.Normal:
								// Draw the faded image on its own
								DrawImageInCentre(g, drawRect, image.ImageSize, image.FadedImage, imageAttr);
								break;
							case ItemState.Pressed:
							...
							case ItemState.Open:
								...
								{
									Rectangle disabledRect = new Rectangle(drawRect.Left + 2, drawRect.Top + 2, drawRect.Width - 2, drawRect.Height - 2);
									Rectangle enabledRect = new Rectangle(drawRect.Left, drawRect.Top, drawRect.Width - 2, drawRect.Height - 2);

									// Draw the disabled image offset down and right
									DrawImageInCentre(g, disabledRect, image.ImageSize, image.GetDisabledImage(baseColor), imageAttr);
									// Draw the normal image offset up and left
									DrawImageInCentre(g, enabledRect, image.ImageSize, image.Image, imageAttr);
								}
								break;
							default:
								// Should never happen!
												...
												Color sepLightColor)
		{
			...
			switch(style)
			{
				case VisualStyle.IDE:
					...
					using(Pen dPen = new Pen(sepDarkColor))
					{            
						if (direction == LayoutDirection.Horizontal)
						{
							// Find point halfway across for separator line
							int xPos = drawRect.Left + (drawRect.Width - 1) / 2;

							// Draw as single vertical line
							g.DrawLine(dPen, xPos, drawRect.Top, xPos, drawRect.Bottom - 1);
						}
						else
						{
							// Find point halfway down for separator line
							int yPos = drawRect.Top + (drawRect.Height - 1) / 2;

							// Draw as single horizontal line
							g.DrawLine(dPen, drawRect.Left, yPos, drawRect.Right - 1, yPos);                    
						}
					}
					break;
				case VisualStyle.Plain:
												  ...
												  Rectangle drawRect)
		{
			if (gradient)
			{
				...
				// Change the width to represent the entire screen size
				parentRect.Size = SystemInformation.VirtualScreen.Size;
