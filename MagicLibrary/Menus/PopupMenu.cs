...
using Junxian.Magic.Controls.Command;
using Junxian.Magic.Win32;
using Microsoft.Win32;

namespace Junxian.Magic.Menus
{
	/// <summary>
	/// Show a collection of MenuCommands as context menu.
	/// </summary>
    [ToolboxItem(false)]
    ...
    public class PopupMenu : NativeWindow, IDisposable
    {
        ...
        private static readonly int[,] _position = { 
														...
														{2, 1, 0, 1, 2, 3, 3, 5, 4, 4, 2, 6, 5, 5, 1, 10, 4, 4, 2, 2, 0, 0},	// Office2003
														{2, 1, 0, 1, 2, 3, 3, 5, 4, 4, 2, 6, 5, 5, 1, 10, 4, 4, 2, 2, 0, 0}		// IDE2005
													};
        // Other class constants
        private static int _selectionDelay = 400;
        private static int _expansionDelay = 1100;
        private static int _imageWidth = 16;
        private static int _imageHeight = 16;
        private static int _shadowLength = 4;
        private static int _blendSteps = 6;
        ...
        private readonly int WM_DISMISS = (int)Win32.Msgs.WM_USER + 1;
        private readonly int WM_OPERATE_SUBMENU = (int)Win32.Msgs.WM_USER + 2;

        // Instance fields
        private Timer _timer;
        private Font _textFont;
        private int _instance;
        private int _popupItem;
        private int _trackItem;
        private int _borderGap;
        private int _returnDir;
        private int _extraSize;
        private int _excludeOffset;
        ...
        private bool _popupDown;
        private bool _popupRight;
		...
		private bool _defaultHighlightColor;
		private bool _eatInitialMouseUp;
		private string name;
		private Color _backColor;
		private Color _backLightColor;
		private Color _backDarkColor;
		private Color _backDarkDarkColor;
		private Color _backLightLightColor;
		private Color _textColor;
        private Color _highlightTextColor;
        private Color _highlightColor;
        private Color _highlightColorDark;
        private Color _highlightColorLight;
        private Color _highlightColorLightLight;
        private Color _controlLL;
        private Size _currentSize;
        private VisualStyle _style;
        private Point _screenPos;
        private Point _lastMousePos;
        private Point _currentPoint;
        private Point _leftScreenPos;
        private Point _aboveScreenPos;
        private LayoutDirection _direction;
        private PopupMenu _parentMenu;
        private PopupMenu _childMenu;
        private SolidBrush _controlLBrush;
        private SolidBrush _controlEBrush;
        private SolidBrush _controlLLBrush;
        private Animate _animate;
        private Animation _animateStyle;
        private ArrayList _drawCommands;
        private MenuControl _parentControl;
        private MenuCommand _returnCommand;
        private MenuCommand _parentCommand;
		...
		/// <summary>
		/// Occurs when the PopupMenu is about to be Tracked.
		/// </summary>
		public event EventHandler TrackingPopup;

        static PopupMenu()
        {
            // Create a strip of images by loading an embedded bitmap resource
            _menuImages = ResourceHelper.LoadBitmapStrip(Type.GetType("Junxian.Magic.Menus.PopupMenu"),
                                                         "Junxian.Magic.Resources.ImagesPopupMenu.bmp",
                                                         new Size(16,16),
                                                         new Point(0,0));
		...
		/// <summary>
		/// Initialize a new instance of the PopupMenu class.
		/// </summary>
        public PopupMenu()
        {
		...
		[Editor("Junxian.Magic.Menus.MenuCollectionEditor", typeof(UITypeEditor))]
		...
		/// <summary>
		/// Gets and sets a value indicating if infrequent items should be displayed.
		/// </summary>
        [Category("Behavour")]
        [DefaultValue(false)]
        ...
        public Color HighlightColor
        {
            ...
            set 
            { 
				DefineHighlightColors(value); 
				_defaultHighlightColor = false;
            }
        }
        
        /// <summary>
        /// Reset the HighlightColor property.
        /// </summary>
		...
		/// <summary>
		/// Gets and sets if the menu pops up on the right hand side of the start point.
		/// </summary>
		[Category("Appearance")]
		[DefaultValue(true)]
		public bool PopupRight
		{
			get { return _popupRight; }
			set { _popupRight = value; }
		}
		
		/// <summary>
		/// This menu should kill itself.
		/// </summary>
		public void Dismiss()
		{
		...
		/// <summary>
		/// Show and track the Popup at a given screen location.
		/// </summary>
		/// <param name="screenPos">Screen coordinates position of menu.</param>
		/// <returns>MenuCommand instance of selected item; otherwise null.</returns>
		...
		/// </summary>
		/// <param name="screenPos">Screen coordinates position of menu.</param>
		/// <param name="selectFirst">Should first item in collection be selected by default.</param>
		/// <returns>MenuCommand instance of selected item; otherwise null.</returns>
        public MenuCommand TrackPopup(Point screenPos, bool selectFirst)
        {
			ValidateState();

			// Raise the TrackPopup event so menu can be populated
			if (TrackingPopup != null)
				TrackingPopup(this, EventArgs.Empty);
        ...
        internal MenuCommand TrackPopup(Point screenPos, 
                                        ...
                                        bool animateIn,
                                        ref int returnDir,
										ref MenuClickEventArgs.ClickSource clickSource)
        {
			ValidateState();
			...
			// Return the reason for any selected command
			clickSource = _clickSource;

            return ret;
        }

        private MenuCommand InternalTrackPopup(Point screenPosTR, 
                                               Point screenPosTL, 
                                               MenuCommandCollection menuCollection, 
        ...
        private MenuCommand InternalTrackPopup(bool selectFirst)
        {
			...
			try
			{
				...
				// If the popup appears on top of the current mouse position eat any mouse up that arrives
				// before a mouse down in case the popup was launched during another controls Mouse Down event.
				if ((mousePos.X >= _currentPoint.X) && 
				    (mousePos.X <= _currentPoint.X + localWidth) &&
					(mousePos.Y >= _currentPoint.Y) && 
					(mousePos.Y <= _currentPoint.Y + + localHeight))
					_eatInitialMouseUp = true;
	...
	//						Console.WriteLine("Track {0} {1}", this.Handle, ((Win32.Msgs)msg.message).ToString());
								...
								switch((int)msg.wParam)
								{
									...
									case (int)Win32.VirtualKeys.VK_RETURN:
										...
										if (_trackItem != -1)
										{
											...
											else
											{
												...
												{
													// Remember the source of the event
													_clickSource = MenuClickEventArgs.ClickSource.Return;
									...
									default:
										...
										if (selectItem != -1)
										{
											...
											// Remember the source of the event
											_clickSource = MenuClickEventArgs.ClickSource.Mnemonic;
			...
			if (!exception)
			{
				...
				if ((_parentMenu == null) && (_returnCommand != null) && (_parentControl == null))
				{
					...
					_returnCommand.OnClick(new MenuClickEventArgs(_clickSource));
				}
			}

			// Make sure we dispose of objects
			Release(false);

			return (exception ? null : _returnCommand);
		}
		...
		private void ManualAnimateBlend(bool show)
		{
        ...
        private void CreateAndShowWindow()
        {
            ...
            _layered = (_allowLayered && !_256Colors &&  _supportsLayered && ((_style == VisualStyle.IDE) || 
        ...
        private void UpdateLayeredWindow(Point point, Size size, byte alpha)
        {
			...
			// Remove no longer needed bitmap
			memoryBitmap.Dispose();
        }

        private void SetWindowRegion(Size winSize)
        {
				...
				(_style == VisualStyle.Office2003))
            {
                ...
                if (!((_direction == LayoutDirection.Vertical) && !_excludeTop))
                {
        ...
        private Point CorrectPositionForScreen(Size winSize)
        {
                ...
                if ((screenPos.Y + winSize.Height) > screenBottom)
                {
                    ...
                    if ((screenPos.Y + winSize.Height) > screenBottom)
                    {
                        ...
                        else
                        {
                            ...
                            {
								using(Graphics g = _parentControl.CreateGraphics())
								{
									// Find size of the parent command
									SizeF size = g.MeasureString(_parentCommand.Text, _parentControl.Font);
									
									// Move across the submenu so it does not cover the parent command
									screenPos.X += (int)size.Width;
									
									// Do not leave a part of the border empty, as we cannot link up with parent
									_borderGap = 0;
								}
                            }
                        }
        ...
        private Size GenerateDrawPositions()
        {
            ...
            int screenHeight = Screen.GetWorkingArea(_currentPoint).Height;
        ...
        private void DefineHighlightColors(Color baseColor)
        {
			...
			{
				// In reduced color mode we need to use alternate system colors
				if (_256Colors)
				{
					_highlightColorDark = SystemColors.Highlight;
					_highlightColorLight = SystemColors.Window;
					_highlightColorLightLight = SystemColors.Window;
				}
				else
				{
					_highlightColorDark = SystemColors.Highlight;
        ...
        private void DefineColors(Color backColor)
        {
			_backColor = backColor;

			// Make sure we dispose of existing brushes
			if (_controlLBrush != null)		
				_controlLBrush.Dispose();
			
			if (_controlEBrush != null)		
				_controlEBrush.Dispose();

			if (_controlLLBrush != null)	
				_controlLLBrush.Dispose();

			// If reduced color mode we need to use system colors for defaults
			if ((_backColor == SystemColors.Control) && _256Colors)
			{
				_controlLL = SystemColors.Window;
				_controlLBrush = new SolidBrush(SystemColors.Control);
				_controlEBrush = new SolidBrush(SystemColors.ControlDark);
				_controlLLBrush = new SolidBrush(_controlLL);

				// Cache variations
				_backLightColor = SystemColors.ControlLight;
				_backDarkColor = SystemColors.ControlDark;
				_backDarkDarkColor = SystemColors.ControlDarkDark; 
				_backLightLightColor = SystemColors.ControlLightLight;
			}
			else
			{
				...
				// Cache variations
				_backLightColor = ControlPaint.Light(_backColor);
				_backDarkColor = ControlPaint.Dark(_backColor);
				_backDarkDarkColor = ControlPaint.DarkDark(_backColor); 
				_backLightLightColor = ControlPaint.LightLight(_backColor);
			}
        }

        private void ApplyVerticalSeparators(int sepHeight)
        {
                ...
                case VisualStyle.IDE:
					{
						...
						using(Pen mainBorder = new Pen(_backDarkColor))
						{
						...
						if ((_borderGap > 0) && (!_excludeTop) && (_direction == LayoutDirection.Horizontal))
						{
                ...
                case VisualStyle.IDE2005:
					{
						...
						if (_defaultBackColor && ((_style == VisualStyle.Office2003) ||
												  (_style == VisualStyle.IDE2005)))
						{
							// Paint the main area background
							using(Brush backBrush = new SolidBrush(_colorDetails.MenuBackColor))
								g.FillRectangle(backBrush, main);

							// Draw single line border around the main area
							using(Pen mainBorder = new Pen(_colorDetails.OpenBorderColor))
								g.DrawRectangle(mainBorder, main);
						}
						else
						{
						...
						if (_defaultBackColor && ((_style == VisualStyle.Office2003) ||
												  (_style == VisualStyle.IDE2005)))
						{
							DrawOfficeIDE2005Column(imageRect, g);
						}
						else
						{
							g.FillRectangle(Brushes.White, imageRect);
        ...
        private void DrawShadowVertical(Graphics g, int left, int top, int width, int height)
        {
            if (_layered)
            {
				Color raw = Color.Black;
            
				// In Office2003 we use a color matching the theme
				if (_defaultBackColor && ((_style == VisualStyle.Office2003) ||
										  (_style == VisualStyle.IDE2005)))
					raw = _colorDetails.OpenBorderColor;
            
                Color extraColor = Color.FromArgb(64, raw);
                Color darkColor = Color.FromArgb(48, raw);
                Color lightColor = Color.FromArgb(0, raw);
            ...
            {
                using(SolidBrush shadowBrush = new SolidBrush(_backDarkColor))
                    g.FillRectangle(shadowBrush, left, top, width, height + 1);
            }
        }

        private void DrawShadowHorizontal(Graphics g, int left, int top, int width, int height, Shadow op)
        {
            if (_layered)
            {
				Color raw = Color.Black;
        ...
        private void DrawColumn(Graphics g, Rectangle main)
        {
		...
		internal void DrawSingleCommand(Graphics g, DrawCommand dc, bool hotCommand)
		{
            ...
            if (dc.Expansion)
            {
                ...
                if ((_style == VisualStyle.IDE) || ((_style == VisualStyle.Office2003) ||
													(_style == VisualStyle.IDE2005)))
                {
                ...
                if (hotCommand)
                {
                    switch(_style)
                    {
                        ...
                        case VisualStyle.IDE2005:
							{
								...
								if (_defaultHighlightColor && ((_style == VisualStyle.Office2003) ||
															   (_style == VisualStyle.IDE2005)))
								{
									// Draw the selection area
									using (SolidBrush selectBrush = new SolidBrush(_colorDetails.TrackMenuInsideColor))
										g.FillRectangle(selectBrush, selectArea);
								}
								else
								{
								...
								if (_defaultHighlightColor && ((_style == VisualStyle.Office2003) ||
															   (_style == VisualStyle.IDE2005)))
								{
									using (Pen selectPen = new Pen(_colorDetails.MenuItemBorderColor))
										g.DrawRectangle(selectPen, selectArea);
								}
								else
								{
                        ...
                        case VisualStyle.Plain:
                            ...
                            using (Pen lightPen = new Pen(_backLightLightColor),
                                       darkPen = new Pen(_backDarkDarkColor))
                            {
                    ...
                    switch(_style)
                    {
                        ...
                        case VisualStyle.IDE2005:
													  ...
													  (_style == VisualStyle.IDE2005)))
							{
								...
								using(Brush backBrush = new SolidBrush(_colorDetails.MenuBackColor))
									g.FillRectangle(backBrush, new Rectangle(drawRect.Left + 1, drawRect.Top,
																			 drawRect.Width - 1, drawRect.Height));

								DrawOfficeIDE2005Column(imageCol, g);
							}
							else
							{
								// Fill the entire drawing area with white
				...
				Image drawImage = _menuImages.Images[(int)ImageIndex.Expansion];
            ...
            else
            {
                    ...
                    if (dc.VerticalSeparator)
                    {
                        switch(_style)
                        {
                            case VisualStyle.IDE:
                                ...
                                using (Pen separatorPen = new Pen(_backDarkColor))
							...
							case VisualStyle.IDE2005:
								...
								if (_defaultBackColor && ((_style == VisualStyle.Office2003) ||
														  (_style == VisualStyle.IDE2005)))
								{
									using (Pen separatorPen = new Pen(_colorDetails.MenuSeparatorColor))
										g.DrawLine(separatorPen, drawRect.Left, drawRect.Top, drawRect.Left, drawRect.Bottom);
								}
								else
								{
                        ...
                        switch(_style)
                        {
                            ...
                            case VisualStyle.Plain:
                                if (dc.Infrequent && _highlightInfrequent)
                                {
                                    ...
                                    using(Brush drawBrush = new SolidBrush(_backLightColor))
                ...
                else
                {
							...
							case VisualStyle.IDE2005:
								{
															  ...
															  (_style == VisualStyle.IDE2005)))
									{
										// Paint the main background color
										using(Brush backBrush = new SolidBrush(_colorDetails.MenuBackColor))
											g.FillRectangle(backBrush, new Rectangle(drawRect.Left + 1, drawRect.Top, drawRect.Width - 1, drawRect.Height));
					...
					// Must remember to dispose of the brush
					textBrush.Dispose();
					...
					if (mc.Checked)
					{
						switch(_style)
						{
							...
							case VisualStyle.IDE2005:
								{
									using(Pen boxPen = new Pen(_colorDetails.MenuItemBorderColor))
									{
										Brush boxBrush;

										if (hotCommand)
											boxBrush = new SolidBrush(_colorDetails.TrackMenuCheckInsideColor);
										else
											boxBrush = new SolidBrush(_colorDetails.MenuCheckInsideColor);
                    ...
                    if (dc.SubMenu)
                    {
                        ...
                        if (mc.Enabled)
                        {
							...
							Image drawImage = _menuImages.Images[subMenuIndex];
                            g.DrawImage(drawImage, subMenuX, imageTop);
							drawImage.Dispose();
                        }
                        else
                        {
                            // Draw a image disabled
							Image drawImage = _menuImages.Images[(int)ImageIndex.SubMenu];
                            ControlPaint.DrawImageDisabled(g, drawImage, subMenuX, imageTop, _highlightTextColor);
							drawImage.Dispose();
                        }
                    }
                }
        ...
        private string GetShortcutText(Shortcut shortcut)
        {
            // Get the key code
        ...
        private int ProcessMnemonicKey(char key)
        {
        ...
        internal Win32.POINT MousePositionToScreen(Win32.MSG msg)
        {
        ...
        internal bool ParentControlWantsMouseMessage(Win32.POINT screenPos, ref Win32.MSG msg)
        {
        ...
        internal PopupMenu WantMouseMessage(Win32.POINT screenPos)
        {
            Win32.RECT rectRaw = new Win32.RECT();
        ...
        private void SwitchSelection(int oldItem, int newItem, bool mouseChange, bool reverting)
        {
        ...
        private void OperateSubMenu(int popupItem, bool selectFirst)
		{
			User32.PostMessage(this.Handle, WM_OPERATE_SUBMENU, (uint)popupItem, (uint)(selectFirst ? 1 : 0));
		}

        private void OnWM_OPERATE_SUBMENU(ref Message m)
        {
			...
			_childMenu.AllowLayered = this.AllowLayered;

			// Need to pass knowledge of defaults
			_childMenu._defaultBackColor = this._defaultBackColor;
			_childMenu._defaultHighlightColor = this._defaultHighlightColor;
            ...
            MenuCommand subMenu = _childMenu.InternalTrackPopup(new Point(screenPosTR.x, screenPosTR.y),
			...
			// If we do not have a command assigned to use, then use returned one from
			// submenu, otherwise we have one because we were clicked with the submenu
			// still visible and so lets use ours instead.
			if (_returnCommand == null)
				_returnCommand = subMenu;
		...
		/// </summary>
		/// <param name="mc">Command that is selected.</param>
        ...
        private void OnWM_ACTIVATEAPP(ref Message m)
        {
        ...
        private void OnWM_MOUSEMOVE(int xPos, int yPos)
        {
        ...
        private void OnWM_YBUTTONUP(int xPos, int yPos)
        {
            // Convert from screen to client coordinates
            xPos -= _currentPoint.X;
            yPos -= _currentPoint.Y;
        ...
        private void OnWM_MOUSEACTIVATE(ref Message m)
        {
        ...
        private void OnWM_SETCURSOR()
        {
            // Always use the arrow cursor
            User32.SetCursor(User32.LoadCursor(IntPtr.Zero, (uint)Win32.Cursors.IDC_ARROW));
        }
	
        private void OnWM_DISMISS()
        {	
        ...
        private bool OnWM_NCHITTEST(ref Message m)
        {
...
//			Console.WriteLine("WndProc(PopupMenu) {0} {1}", this.Handle, ((Win32.Msgs)m.Msg).ToString());
        ...
        private static Bitmap GetShadowCache(int width, int height)
        {
		...
		private void DrawOfficeIDE2005Column(Rectangle rect, Graphics g)
		{
			using(LinearGradientBrush openBrush = new LinearGradientBrush(rect, _colorDetails.ColumnBaseColor1, _colorDetails.ColumnBaseColor2, 0f))
			{
				if (_style == VisualStyle.IDE2005)
				{
					Blend blend = new Blend();
					blend.Factors = new float[]{0f, 0.5f, 0.55f, 1f};
					blend.Positions = new float[]{0f, 0.66f, 0.67f, 1f};
					openBrush.Blend = blend;
				}
		...
		private void ValidateState()
		{
			...
			DefineColors(BackColor);
		}

		internal void Release(bool all)
		{
			// Make sure we dispose of objects
			if (_controlLBrush != null)
			{
			...
			if (all && _textFont != null)
			{
				_textFont.Dispose();
				_textFont = null;
			}
			
			if (all && _menuCommands != null)
			{
